...
export const metadata: Metadata = { title: "Revisões · Bora Estudar" };
...
export default async function TeacherReviewsPage() {
    ...
    students
      ...
      .map(async ({ profile, activePlan }) => {
        ...
        const needing = progress.blocks
          ...
          .map((b) => ({
            block: b.block_id ? blockById.get(b.block_id) : null,
  ...
  const withPending = rows.filter((r) => r.needing.length > 0);
